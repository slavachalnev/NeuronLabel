...
const data = [
  {
    ...
    "snippets": [
      {
        "text": "This is a text snippet with an activated token.",
        "activations": [0, 0, 0.5, 0, 0, 0, 0]
      },
      {
        "text": "A different activated token.",
        "activations": [0, 0.8, 0.5]
      }
    ]
  }
];
...
function renderSnippets() {
  data[0].snippets.forEach((snippet) => {
    const snippetDiv = document.createElement("div");
    snippetDiv.className = "snippet";
    snippet.text.split(" ").forEach((word, index) => {
      const token = document.createElement("span");
      token.className = "token";
      token.style.backgroundColor = `rgba(255, 0, 0, ${snippet.activations[index]})`;
      token.textContent = word + " ";
      snippetDiv.appendChild(token);
    });
